...
from modeltask2 import FusionLayer1,  Model1, Model2, Model3, Model4
...
def train_gpu(train_x, train_y, test_x, test_y, net, real_edge_1, real_edge_2, real_edge_3, real_edge_4):
    ...
    # 改进学习率调度策略：使用ReduceLROnPlateau，根据验证集性能自动调整
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        opti, mode='max', factor=0.5, patience=5, verbose=True, min_lr=1e-6
    )
    ...
    for epoch in range(args.epoches):
        ...
        for x, y in train_iter:
            ...
            # 动态调整损失权重：随着训练进行，逐渐增加主任务权重
            main_weight = 30 + (epoch / args.epoches) * 20  # 从30逐渐增加到50
            aux_weight = max(0.5 - (epoch / args.epoches) * 0.3, 0.2)  # 从0.5逐渐减少到0.2
            
            l = main_weight * loss_function(final_embed, train_label) + \
                aux_weight * loss_function(z1, real_edge_1) + \
...
def main():
    ...
    save_result(f"result/{split_id}/task2_m1234", "all", result_all)
