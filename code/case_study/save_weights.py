...
from model_case import FusionLayer1,  Model1, Model2, Model3, Model4, Model5
...
parser.add_argument("--n_drug", type=int, default=1409)
...
def train_gpu(train_x, train_y, test_x, test_y, net, real_edge_1, real_edge_2, real_edge_3, real_edge_4):
    ...
    # ========= 新增：记录最好模型 & 早停参数 =========
    best_test_acc = 0.0                         # 当前最优 test_acc
    max_test_output = None                      # 与你的返回值保持一致
    best_model_path = f"datasets/case_study/case{case_id}/task2test_bestmodel_case_label0"
    os.makedirs(os.path.dirname(best_model_path), exist_ok=True)

    patience = getattr(args, "patience", 10)    # 早停耐心；若 args 无该字段，则默认 10
    min_delta = getattr(args, "min_delta", 0.0) # 最小提升阈值：提升需 >= min_delta 才算改进
    epochs_no_improve = 0
    best_epoch = -1
    ...
    for epoch in range(args.epoches):
        ...
        with torch.no_grad():
            ...
            # CrossEntropyLoss 直接用 logits
            test_logits = test_final_embed
            loss = loss_function(test_logits, test_label)
            test_loss = loss.item()

            test_output = F.softmax(test_logits, dim=1).to("cpu")
            test_score = f1_score(torch.argmax(test_output, dim=1).cpu().numpy(),
                                  test_label.cpu().numpy(),
                                  average='macro')
            test_acc = accuracy_score(torch.argmax(test_output, dim=1).cpu().numpy(),
                                      test_label.cpu().numpy())
            ...
            if (test_acc - best_test_acc) >= min_delta:
                ...
                print(f"===> New best model saved at epoch {best_epoch}, test_acc: {best_test_acc:.4f}")
            else:
                # 没有提升
                epochs_no_improve += 1
    ...
    # 若训练过程中从未有提升，确保 max_test_output 有值（以免返回时报错）
    if max_test_output is None:
        with torch.no_grad():
            test_x_gpu = torch.LongTensor(test_x).to(device)
            test_label = torch.LongTensor(test_y).to(device)
            logits, *_ = net(test_x_gpu)
            max_test_output = F.softmax(logits, dim=1).to("cpu")

    return (test_loss / len(test_y),
            max(test_list) if len(test_list) > 0 else 0.0,
            train_l / len(train_y),
...
def main():
    ...
    # === 读取训练集 ===
    train_path ="datasets/case_study/train_dataset.csv"
    ...
    # === 读取测试集 ===
    test_path = f"datasets/case_study/case{case_id}/task2test_dataset_case_label0.csv"
